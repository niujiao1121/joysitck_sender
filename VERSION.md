...
### 0.2.4
- Linux/Windows 手柄发送器控制台输出改为动态状态面板，实时显示最新 CMD、轴值、原始输入、姿态、模式和最近事件
- 保留文件日志；非交互终端下继续使用普通日志输出

### 0.2.5
- `joystick_sender` 新增 `gait_default`，启动默认步态可配置
- X/Y 改为固定输出 `gait=0` / `gait=1`

### 0.2.3
- 启动时打印当前操作方式，包含速度、姿态、模式和步态按键映射

### 0.2.2
- 调整手柄姿态映射：LB/RB 控制 pitch，十字键左右控制 roll
